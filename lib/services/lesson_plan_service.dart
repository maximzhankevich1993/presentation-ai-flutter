...
class _LessonConstructorScreenState extends State<LessonConstructorScreen> {
  ...
  // Список стандартов из LessonPlanService
  final List<Map<String, String>> _standards = LessonPlanService.standards;
  
  // Группировка стандартов по регионам
  Map<String, List<Map<String, String>>> get _groupedStandards {
    final Map<String, List<Map<String, String>>> groups = {};
    for (final standard in _standards) {
      final region = standard['region'] ?? 'Другие';
      if (!groups.containsKey(region)) {
        groups[region] = [];
      }
      groups[region]!.add(standard);
    }
    return groups;
  }
  ...
  Future<void> _generateLesson() async {
    ...
    try {
      // Используем существующий LessonPlanService
  ...
  Presentation _convertToPresentation(LessonPlan lessonPlan) {
    ...
    slides.add(Slide(
      title: 'План урока',
      content: [
        '📚 ${_getSubjectIcon(lessonPlan.subject)} ${lessonPlan.subject}',
                        ...
                        child: Column(
                          children: [
                            _buildTextField(
                              ...
                              hint: 'Тема урока (например: Дроби, Present Simple...)',
                              icon: Icons.topic_rounded,
                            ),
                            const SizedBox(height: 12),
                            _buildTextField(
                              controller: _subjectController,
                              hint: 'Предмет (например: Математика, English...)',
                              icon: Icons.subject_rounded,
                            ),
                            const SizedBox(height: 12),
                            _buildTextField(
                              controller: _gradeController,
                              hint: 'Класс (например: 5, 10, 1st grade...)',
