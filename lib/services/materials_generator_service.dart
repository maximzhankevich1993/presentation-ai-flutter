...
class MaterialsGeneratorService {
  ...
  static String _generateLinkedInPost(Presentation presentation) {
    final firstSlide = presentation.slides.isNotEmpty
        ? presentation.slides.first
        : null;

    final keyPoints = presentation.slides
        .take(3)
        .where((s) => s.content.isNotEmpty)
        .map((s) => s.content.first)
        .toList();
...
🔥 Главные инсайты:
${keyPoints.isNotEmpty ? keyPoints.map((p) => '• $p').join('\n') : '• Нет данных'}

💡 Мой главный вывод:
${firstSlide?.content.isNotEmpty == true
...
''';
  ...
  static String _generateTwitterThread(Presentation presentation) {
    ...
    tweets.add(
      'Конец треда! Полная презентация доступна по ссылке 🔗\n\nСоздано в @PresentationAI ✨',
    );
...
## Введение
${slides.isNotEmpty ? slides.first.content.join('\n') : 'Нет данных'}

${slides.length > 2
        ? slides
            .skip(1)
            .take(slides.length - 2)
            .map(
              (slide) => '''
## ${slide.title}
...
## Заключение
${slides.isNotEmpty ? slides.last.content.join('\n') : 'Нет данных'}
