...
class _SettingsScreenState extends State<SettingsScreen> with SingleTickerProviderStateMixin {
  ...
  Widget _buildColorGrid(UserProvider userProvider) {
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isFree = color['free'] as bool;
        final isAvailable = isFree || userProvider.isPremium;
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            child: !isAvailable
                ? Center(
                    child: Icon(Icons.lock, size: 18, color: Colors.amber[700]),
                  )
                : null,
          ),
        );
  ...
  Widget _buildGradientGrid(UserProvider userProvider) {
    final gradients = [
      ...
      {'name': 'Фиолетовый', 'colors': [0xFFB224EF, 0xFF7579FF], 'free': false},
      {'name': 'Мохито', 'colors': [0xFF11998E, 0xFF38EF7D], 'free': false},
      {'name': 'Кровавая луна', 'colors': [0xFFCB356B, 0xFFBD3F32], 'free': false},
    ];
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isFree = gradient['free'] as bool;
        final isAvailable = isFree || userProvider.isPremium;
        final colors = (gradient['colors'] as List).map((c) => Color(c as int)).toList();
        
        return GestureDetector(
          onTap: isAvailable ? () => _selectGradient(gradient['name'] as String) : () => _showPremiumNudge(),
          child: Container(
            decoration: BoxDecoration(
  ...
  Widget _buildTextureGrid(UserProvider userProvider) {
    final textures = [
      ...
      {'name': 'Холст', 'icon': '🖼', 'free': false},
      {'name': 'Точки', 'icon': '⚫', 'free': false},
    ];
    ...
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final isFree = texture['free'] as bool;
        final isAvailable = isFree || userProvider.isPremium;
        
        return GestureDetector(
          onTap: isAvailable ? () => _selectTexture(texture['name'] as String) : () => _showPremiumNudge(),
          child: Container(
            decoration: BoxDecoration(
              ...
              children: [
                Center(
                  child: Column(
                    ...
                    children: [
                      Text(texture['icon'] as String, style: TextStyle(fontSize: 28.sp)),
                      SizedBox(height: 4.h),
                      Text(
                ...
                if (!isAvailable)
                  Positioned(
                    top: 4,
                    right: 4,
                    child: Icon(Icons.lock, size: 12, color: Colors.amber[700]),
                  ),
              ],
            ),
          ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 8.h),
          Text(
            'Выбери сочетание шрифтов для заголовков и текста',
              ...
              child: Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.crown, color: Color(0xFFF59E0B)),
                  ),
                  SizedBox(width: 16.w),
                  Expanded(
                    child: Text(
                      'Разблокируй все настройки с Premium',
  ...
  void _selectBackground(int colorValue) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Фон выбран'),
        duration: Duration(seconds: 1),
      ),
    );
  }

  void _selectGradient(String name) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Градиент "$name" выбран'),
  ...
  void _selectTexture(String name) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Текстура "$name" выбрана'),
        duration: const Duration(seconds: 1),
      ),
    );
  }

  void _selectFont(String name) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text('Шрифт "$name" выбран'),
        duration: const Duration(seconds: 1),
      ),
    );
  }
