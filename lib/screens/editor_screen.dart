...
class _EditorScreenState extends State<EditorScreen> with TickerProviderStateMixin {
  ...
  @override void initState() {
    super.initState(); _p = widget.presentation; _ci = List.filled(_p.slides.length, null); _cb = List.filled(_p.slides.length, null);
  ...
  Future<void> _loadImages() async { for (int i = 0; i < _p.slides.length; i++) { _ai[i] = await ImageService.searchImage(_tc[i].text.isNotEmpty ? _tc[i].text : _p.title); if (mounted) setState(() {}); } }
  void _save() { for (int i = 0; i < _p.slides.length; i++) { _p.slides[i].title = _tc[i].text; _p.slides[i].content = _cc[i].map((c) => c.text).toList(); } }
  void _addSlide() { final up = Provider.of<UserProvider>(context, listen: false); if (_p.slides.length >= up.maxSlidesPerPresentation) { _toast('Максимум ${up.maxSlidesPerPresentation} слайдов'); return; } setState(() { final x = _as + 1; _p.slides.insert(x, Slide(title: 'Новый слайд', content: ['Введите текст'])); _tc.insert(x, TextEditingController(text: 'Новый слайд')); _cc.insert(x, [TextEditingController(text: 'Введите текст')]); _ci.insert(x, null); _cb.insert(x, null); _fs.insert(x, 9.0); _fn.insert(x, _gf); _sfc.insert(x, null); _tr.insert(x, 'none'); _as = x; }); }
  void _delSlide(int i) { if (_p.slides.length <= 1) return; setState(() { _tc[i].dispose(); for (var c in _cc[i]) c.dispose(); _p.slides.removeAt(i); _tc.removeAt(i); _cc.removeAt(i); _ci.removeAt(i); _cb.removeAt(i); _fs.removeAt(i); _fn.removeAt(i); _sfc.removeAt(i); _tr.removeAt(i); if (_as >= _p.slides.length) _as = _p.slides.length - 1; }); _countUp(); }
  void _dupSlide(int i) { setState(() { final x = i + 1; _p.slides.insert(x, Slide(title: _p.slides[i].title, content: List.from(_p.slides[i].content))); _tc.insert(x, TextEditingController(text: _tc[i].text)); _cc.insert(x, _cc[i].map((c) => TextEditingController(text: c.text)).toList()); _ci.insert(x, _ci[i]); _cb.insert(x, _cb[i]); _fs.insert(x, _fs[i]); _fn.insert(x, _fn[i]); _sfc.insert(x, _sfc[i]); _tr.insert(x, _tr[i]); _as = x; }); _countUp(); }
  void _moveSlide(int f, int t) { if (t < 0 || t >= _p.slides.length) return; setState(() { void sw<T>(List<T> l) { final x = l[f]; l[f] = l[t]; l[t] = x; } sw(_p.slides); sw(_tc); sw(_cc); sw(_ci); sw(_cb); sw(_fs); sw(_fn); sw(_sfc); sw(_tr); _as = t; }); }
  ...
  Future<void> _improveSlide(int i) async { setState(() => _ii = true); try { final t = await AiImproveService.improveText(_tc[i].text); final cs = <String>[]; for (final c in _cc[i]) cs.add(await AiImproveService.improveText(c.text)); if (!mounted) return; setState(() { _tc[i].text = t; for (int j = 0; j < cs.length && j < _cc[i].length; j++) _cc[i][j].text = cs[j]; }); _toast('Текст улучшен', success: true); } catch (e) { _toast('Ошибка: $e', error: true); } finally { if (mounted) setState(() => _ii = false); } }
  Future<void> _uploadImage(int i) async { if (!Provider.of<UserProvider>(context, listen: false).isPremium) { _toast('Замена картинок — Premium', warning: true); return; } final inp = html.FileUploadInputElement()..accept = 'image/*'; inp.click(); inp.onChange.listen((e) { final f = inp.files?.first; if (f == null) return; final r = html.FileReader(); r.readAsDataUrl(f); r.onLoad.listen((_) => setState(() => _ci[i] = r.result as String)); }); }
  Future<void> _uploadBg(int i) async { final p = Provider.of<UserProvider>(context, listen: false).isPremium; final u = _cb.where((b) => b != null).length; if (!p && u >= 10 && _cb[i] == null) { _toast('10 фонов бесплатно', warning: true); return; } final inp = html.FileUploadInputElement()..accept = 'image/*'; inp.click(); inp.onChange.listen((e) { final f = inp.files?.first; if (f == null) return; final r = html.FileReader(); r.readAsDataUrl(f); r.onLoad.listen((_) => setState(() => _cb[i] = r.result as String)); }); }
  Decoration _deco(int i) { if (_cb[i] != null) return BoxDecoration(image: DecorationImage(image: NetworkImage(_cb[i]!), fit: BoxFit.cover), borderRadius: BorderRadius.circular(12)); final bg = _fb[_sbi.clamp(0, _fb.length - 1)]; if (bg['type'] == 'gradient') return BoxDecoration(gradient: LinearGradient(colors: bg['colors'] as List<Color>, begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(12)); return BoxDecoration(color: bg['color'] as Color, borderRadius: BorderRadius.circular(12)); }
...
class _TopBar extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) => Container(height: 52, color: _T.bgSurface, padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(children: [
    _IconBtn(Icons.arrow_back_ios_rounded, onBack, tooltip: 'Назад', s: 17), const SizedBox(width: 8),
...
class _SlideThumbnailState extends State<_SlideThumbnail> {
  ...
  Widget _ev() => Row(children: [
    ...
    if (_h) PopupMenuButton<String>(padding: EdgeInsets.zero, iconSize: 14, color: _T.bgCard, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: const BorderSide(color: _T.border)), icon: const Icon(Icons.more_vert_rounded, color: _T.txtSecondary, size: 14), onSelected: (v) { if (v == 'dup') widget.onDuplicate(); if (v == 'del') widget.onDelete?.call(); if (v == 'up') widget.onMoveUp?.call(); if (v == 'down') widget.onMoveDown?.call(); }, itemBuilder: (_) => [
      ...
      if (widget.onDelete != null) const PopupMenuItem(value: 'del', height: 36, child: _MenuItem(Icons.delete_outline_rounded, 'Удалить', d: true)),
    ]),
  ]);
}
...
class _PropertiesPanel extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) => Container(color: _T.bgSurface, child: Column(children: [
    ...
    const Divider(color: _T.border, height: 1),
    Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(14), child: switch (activeTab) { 'image' => _ImageTab(onUpload: onImageUpload, isPremium: isPremium), 'ai' => _AiTab(isImproving: isImproving, onImprove: onImproveSlide), _ => _DesignTab(globalFont: globalFont, selectedBgIndex: selectedBgIndex, freeBgs: freeBgs, premiumBgs: premiumBgs, customBg: customBg, fontSize: fontSize, fontColor: fontColor, transition: transition, allTransitions: allTransitions, isPremium: isPremium, onBgSelect: onBgSelect, onBgUpload: onBgUpload, onFontChange: onFontChange, onFontSizeChange: onFontSizeChange, onFontColorChange: onFontColorChange, onTransitionChange: onTransitionChange) })),
  ]));
}
...
class _ExportSheet extends StatelessWidget {
  ...
  @override Widget build(BuildContext c) => Container(margin: const EdgeInsets.fromLTRB(16, 0, 16, 32), decoration: BoxDecoration(color: _T.bgSurface, borderRadius: BorderRadius.circular(16), border: Border.all(color: _T.border)), child: Column(mainAxisSize: MainAxisSize.min, children: [
...
class _IconBtn extends StatefulWidget {
  final IconData i; final VoidCallback? o; final double s; final String? tooltip; final bool d, a, dis;
