...
import 'home_screen.dart'; // ← для LogoProvider
...
class _SlideNavigator extends StatelessWidget {
  ...
  final ValueChanged<int> onSelect; final VoidCallback onAdd; final ValueChanged<int> onDelete, onDuplicate, onMoveUp, onMoveDown;
  ...
  Color _getThumbnailColor(int i) { if (customBgs[i] != null) return Colors.grey.shade800; final bg = backgrounds[selectedBgIndex]; if (bg['type'] == 'solid') return bg['color'] as Color; return (bg['colors'] as List<Color>).first; }
...
class _SlideThumbnail extends StatefulWidget { final int index; final String title; final bool isActive, collapsed; final Color bgColor; final VoidCallback onTap, onDuplicate; final VoidCallback? onDelete, onMoveUp, onMoveDown; const _SlideThumbnail({required this.index, required this.title, required this.isActive, required this.collapsed, required this.bgColor, required this.onTap, this.onDelete, required this.onDuplicate, this.onMoveUp, this.onMoveDown}); @override State<_SlideThumbnail> createState() => _SlideThumbnailState(); }
class _SlideThumbnailState extends State<_SlideThumbnail> { bool _hovered = false; @override Widget build(BuildContext context) => MouseRegion(cursor: SystemMouseCursors.click, onEnter: (_) => setState(() => _hovered = true), onExit: (_) => setState(() => _hovered = false), child: GestureDetector(onTap: widget.onTap, child: AnimatedContainer(duration: _T.fast, margin: const EdgeInsets.only(bottom: 6), padding: const EdgeInsets.all(6), decoration: BoxDecoration(color: widget.isActive ? _T.accent.withOpacity(0.12) : _hovered ? _T.bgHover : Colors.transparent, borderRadius: BorderRadius.circular(8), border: Border.all(color: widget.isActive ? _T.accent.withOpacity(0.5) : Colors.transparent, width: 1.5)), child: widget.collapsed ? _collapsedView() : _expandedView()))); Widget _collapsedView() => Column(children: [Container(width: 30, height: 20, decoration: BoxDecoration(color: widget.bgColor, borderRadius: BorderRadius.circular(3)), child: Center(child: Text('${widget.index + 1}', style: TextStyle(fontSize: 8, color: widget.bgColor.computeLuminance() > 0.5 ? Colors.black : Colors.white, fontWeight: FontWeight.w700))))]); Widget _expandedView() => Row(children: [Container(width: 52, height: 34, decoration: BoxDecoration(color: widget.bgColor, borderRadius: BorderRadius.circular(4)), child: Center(child: Text('${widget.index + 1}', style: TextStyle(fontSize: 10, color: widget.bgColor.computeLuminance() > 0.5 ? Colors.black54 : Colors.white38, fontWeight: FontWeight.w700)))), const SizedBox(width: 8), Expanded(child: Text(widget.title.isEmpty ? 'Слайд ${widget.index + 1}' : widget.title, style: TextStyle(color: widget.isActive ? _T.txtPrimary : _T.txtSecondary, fontSize: 11, fontWeight: widget.isActive ? FontWeight.w600 : FontWeight.w400), maxLines: 2, overflow: TextOverflow.ellipsis)), if (_hovered) PopupMenuButton<String>(padding: EdgeInsets.zero, iconSize: 14, color: _T.bgCard, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: const BorderSide(color: _T.border)), icon: const Icon(Icons.more_vert_rounded, color: _T.txtSecondary, size: 14), onSelected: (v) { if (v == 'dup') widget.onDuplicate(); if (v == 'del') widget.onDelete?.call(); if (v == 'up') widget.onMoveUp?.call(); if (v == 'down') widget.onMoveDown?.call(); }, itemBuilder: (_) => [if (widget.onMoveUp != null) const PopupMenuItem(value: 'up', height: 36, child: _MenuItem(Icons.arrow_upward_rounded, 'Вверх')), if (widget.onMoveDown != null) const PopupMenuItem(value: 'down', height: 36, child: _MenuItem(Icons.arrow_downward_rounded, 'Вниз')), const PopupMenuItem(value: 'dup', height: 36, child: _MenuItem(Icons.copy_rounded, 'Дублировать')), if (widget.onDelete != null) const PopupMenuItem(value: 'del', height: 36, child: _MenuItem(Icons.delete_outline_rounded, 'Удалить', danger: true))])]); }
class _MenuItem extends StatelessWidget { final IconData icon; final String label; final bool danger; const _MenuItem(this.icon, this.label, {this.danger = false}); @override Widget build(BuildContext context) => Row(children: [Icon(icon, size: 14, color: danger ? _T.danger : _T.txtSecondary), const SizedBox(width: 8), Text(label, style: TextStyle(color: danger ? _T.danger : _T.txtPrimary, fontSize: 12))]); }

// ═══════════════════════════════════════════════════════════════════════════════
// CANVAS — добавлен логотип
// ═══════════════════════════════════════════════════════════════════════════════
class _Canvas extends StatelessWidget {
  ...
  const _Canvas({super.key, required this.index, required this.titleCtrl, required this.contentCtrl, required this.decoration, required this.isDark, required this.image, required this.font, required this.fontSize, required this.fontColor, required this.slideCount, required this.onAddItem, required this.onRemoveItem, required this.onRemoveImage, required this.hasCustomImage});

  @override
  Widget build(BuildContext context) {
    // Логотип из провайдера
    final logo = context.watch<LogoProvider>().logoUrl;
        ...
        child: SingleChildScrollView(
          ...
          child: Column(children: [
                  ...
                  Padding(
                    ...
                    child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        ...
                        ...contentCtrl.take(5).mapIndexed((i, c) => Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Padding(padding: EdgeInsets.only(top: fontSize * 0.38, right: 7), child: Container(width: 5, height: 5, decoration: const BoxDecoration(color: _T.accent, shape: BoxShape.circle))), Expanded(child: EditableText(controller: c, focusNode: FocusNode(), style: TextStyle(fontSize: fontSize * 1.4, fontFamily: font, color: fontColor.withOpacity(0.8), height: 1.4), cursorColor: _T.accent, backgroundCursorColor: _T.accent, maxLines: 3))]))),
                      ])),
                      if (image != null) ...[const SizedBox(width: 20), Stack(children: [ClipRRect(borderRadius: BorderRadius.circular(10), child: Image.network(image!, width: width * 0.28, height: height * 0.55, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const SizedBox())), if (hasCustomImage) Positioned(top: 4, right: 4, child: GestureDetector(onTap: onRemoveImage, child: Container(width: 20, height: 20, decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(10)), child: const Icon(Icons.close_rounded, color: Colors.white, size: 12))))])],
                    ]),
                  ),
            ...
            LayoutBuilder(builder: (ctx, _) {
              final width = (MediaQuery.of(context).size.width - 460).clamp(360.0, 900.0);
              return Container(width: width, padding: const EdgeInsets.all(16), decoration: BoxDecoration(color: _T.bgSurface, borderRadius: _T.r12, border: Border.all(color: _T.border)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _Tab extends StatelessWidget { final String id, label, active; final IconData icon; final ValueChanged<String> onTap; const _Tab(this.id, this.label, this.icon, this.active, this.onTap); @override Widget build(BuildContext context) { final a = id == active; return Expanded(child: GestureDetector(onTap: () => onTap(id), child: AnimatedContainer(duration: _T.fast, margin: const EdgeInsets.symmetric(horizontal: 2, vertical: 6), padding: const EdgeInsets.symmetric(horizontal: 4), decoration: BoxDecoration(color: a ? _T.accentDim : Colors.transparent, borderRadius: BorderRadius.circular(6), border: Border.all(color: a ? _T.accent.withOpacity(0.3) : Colors.transparent)), child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(icon, size: 12, color: a ? _T.accentLight : _T.txtMuted), const SizedBox(width: 4), Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: a ? _T.accentLight : _T.txtMuted))])))); } }

class _DesignTab extends StatelessWidget {
  final String globalFont; final int selectedBgIndex; final List<Map<String, dynamic>> freeBgs, premiumBgs, allTransitions; final String? customBg; final double fontSize; final Color fontColor; final String transition; final bool isPremium; final ValueChanged<int> onBgSelect; final VoidCallback onBgUpload; final ValueChanged<String> onFontChange, onTransitionChange; final ValueChanged<double> onFontSizeChange; final ValueChanged<Color> onFontColorChange;
  const _DesignTab({required this.globalFont, required this.selectedBgIndex, required this.freeBgs, required this.premiumBgs, required this.customBg, required this.fontSize, required this.fontColor, required this.transition, required this.allTransitions, required this.isPremium, required this.onBgSelect, required this.onBgUpload, required this.onFontChange, required this.onFontSizeChange, required this.onFontColorChange, required this.onTransitionChange});
  static const _fc = [Colors.white, Color(0xFFF2F2F2), Color(0xFF1A1A2E), Color(0xFF000000), Color(0xFF1DB954), Color(0xFF4facfe), Color(0xFFf5576c), Color(0xFFFFD700), Color(0xFFf093fb), Color(0xFFFF6B35)];

  @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    _SectionLabel('ШРИФТ'), const SizedBox(height: 8),
    ...(['Inter', 'Georgia', 'Courier'].map((f) => MouseRegion(cursor: SystemMouseCursors.click, child: GestureDetector(onTap: () => onFontChange(f), child: AnimatedContainer(duration: _T.fast, margin: const EdgeInsets.only(bottom: 6), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 9), decoration: BoxDecoration(color: globalFont == f ? _T.accentDim : _T.bgCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: globalFont == f ? _T.accent.withOpacity(0.4) : _T.border)), child: Row(children: [Expanded(child: Text(f, style: TextStyle(fontFamily: f, color: _T.txtPrimary, fontSize: 13, fontWeight: FontWeight.w600))), if (globalFont == f) const Icon(Icons.check_circle_rounded, color: _T.accent, size: 16)])))))),
    const SizedBox(height: 18), _SectionLabel('РАЗМЕР ТЕКСТА'), const SizedBox(height: 10),
    Row(children: [Expanded(child: SliderTheme(data: SliderThemeData(activeTrackColor: _T.accent, inactiveTrackColor: _T.border, thumbColor: _T.accent, overlayColor: _T.accentDim, trackHeight: 3, thumbShape: const RoundSliderThumbShape(enabledThumbRadius: 6)), child: Slider(value: fontSize, min: 6, max: 18, divisions: 12, onChanged: onFontSizeChange))), Container(width: 36, padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4), decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(6), border: Border.all(color: _T.border)), child: Text('${fontSize.toInt()}', style: const TextStyle(color: _T.txtPrimary, fontSize: 11, fontWeight: FontWeight.w600), textAlign: TextAlign.center))]),
    const SizedBox(height: 18), _SectionLabel('ЦВЕТ ТЕКСТА'), const SizedBox(height: 10),
    Wrap(spacing: 7, runSpacing: 7, children: _fc.map((c) { final s = fontColor.value == c.value; return MouseRegion(cursor: SystemMouseCursors.click, child: GestureDetector(onTap: () => onFontColorChange(c), child: AnimatedContainer(duration: _T.fast, width: 28, height: 28, decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: Border.all(color: s ? _T.accent : Colors.white12, width: s ? 2.5 : 1), boxShadow: s ? [BoxShadow(color: _T.accent.withOpacity(0.4), blurRadius: 6)] : null), child: s ? const Icon(Icons.check_rounded, size: 14, color: Colors.white) : null))); }).toList()),
    const SizedBox(height: 18), _SectionLabel('ФОН'), const SizedBox(height: 8),
    GridView.count(crossAxisCount: 4, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 6, crossAxisSpacing: 6, childAspectRatio: 1.4, children: freeBgs.asMap().entries.map((e) { final i = e.key; final bg = e.value; final s = i == selectedBgIndex && customBg == null; return MouseRegion(cursor: SystemMouseCursors.click, child: GestureDetector(onTap: () => onBgSelect(i), child: AnimatedContainer(duration: _T.fast, decoration: BoxDecoration(gradient: bg['type'] == 'gradient' ? LinearGradient(colors: bg['colors'] as List<Color>, begin: Alignment.topLeft, end: Alignment.bottomRight) : null, color: bg['type'] == 'solid' ? bg['color'] as Color : null, borderRadius: BorderRadius.circular(6), border: Border.all(color: s ? _T.accent : Colors.transparent, width: 2), boxShadow: s ? [BoxShadow(color: _T.accent.withOpacity(0.35), blurRadius: 6)] : null), child: s ? const Center(child: Icon(Icons.check_rounded, color: Colors.white, size: 12)) : null))); }).toList()),
    const SizedBox(height: 8), MouseRegion(cursor: SystemMouseCursors.click, child: GestureDetector(onTap: onBgUpload, child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 9), decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: _T.border)), child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(Icons.upload_rounded, color: _T.txtSecondary, size: 13), SizedBox(width: 6), Text('Загрузить фон', style: TextStyle(color: _T.txtSecondary, fontSize: 12, fontWeight: FontWeight.w500))])))),
    const SizedBox(height: 12), Row(children: [_SectionLabel('PREMIUM ФОНЫ'), const Spacer(), if (!isPremium) Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: _T.gold.withOpacity(0.12), borderRadius: BorderRadius.circular(4)), child: const Text('PRO', style: TextStyle(color: _T.gold, fontSize: 9, fontWeight: FontWeight.w800)))]), const SizedBox(height: 8),
    GridView.count(crossAxisCount: 4, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 6, crossAxisSpacing: 6, childAspectRatio: 1.4, children: premiumBgs.asMap().entries.map((e) { final bg = e.value; return MouseRegion(cursor: isPremium ? SystemMouseCursors.click : SystemMouseCursors.forbidden, child: GestureDetector(onTap: isPremium ? () => onBgSelect(freeBgs.length + e.key) : null, child: Stack(children: [Container(decoration: BoxDecoration(gradient: bg['type'] == 'gradient' ? LinearGradient(colors: bg['colors'] as List<Color>) : null, color: bg['type'] == 'solid' ? bg['color'] as Color : null, borderRadius: BorderRadius.circular(6))), if (!isPremium) Container(decoration: BoxDecoration(color: Colors.black54, borderRadius: BorderRadius.circular(6)), child: const Center(child: Icon(Icons.lock_rounded, color: Colors.white54, size: 13)))])),); }).toList()),
    const SizedBox(height: 18), Row(children: [_SectionLabel('ПЕРЕХОД'), const Spacer(), if (!isPremium) Container(padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 2), decoration: BoxDecoration(color: _T.gold.withOpacity(0.12), borderRadius: BorderRadius.circular(4)), child: const Text('2 бесплатно', style: TextStyle(color: _T.gold, fontSize: 9, fontWeight: FontWeight.w700)))]), const SizedBox(height: 8),
    GridView.count(crossAxisCount: 3, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), mainAxisSpacing: 6, crossAxisSpacing: 6, childAspectRatio: 2.0, children: allTransitions.map((t) { final pr = t['premium'] as bool; final locked = pr && !isPremium; final s = transition == t['id']; return MouseRegion(cursor: locked ? SystemMouseCursors.forbidden : SystemMouseCursors.click, child: GestureDetector(onTap: locked ? null : () => onTransitionChange(t['id'] as String), child: AnimatedContainer(duration: _T.fast, decoration: BoxDecoration(color: s ? _T.accentDim : _T.bgCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: s ? _T.accent.withOpacity(0.5) : _T.border)), child: Stack(alignment: Alignment.center, children: [Column(mainAxisAlignment: MainAxisAlignment.center, children: [Icon(t['icon'] as IconData, size: 16, color: locked ? _T.txtMuted : s ? _T.accent : _T.txtSecondary), const SizedBox(height: 3), Text(t['label'] as String, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: locked ? _T.txtMuted : s ? _T.accent : _T.txtSecondary))]), if (locked) Positioned(top: 4, right: 4, child: Icon(Icons.lock_rounded, size: 9, color: _T.gold.withOpacity(0.7)))])))); }).toList()),
  ]);
}

class _ImageTab extends StatelessWidget { final VoidCallback onUpload; final bool isPremium; const _ImageTab({required this.onUpload, required this.isPremium}); @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_SectionLabel('ИЗОБРАЖЕНИЕ НА СЛАЙДЕ'), const SizedBox(height: 8), if (!isPremium) Container(width: double.infinity, margin: const EdgeInsets.only(bottom: 12), padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10), decoration: BoxDecoration(color: _T.gold.withOpacity(0.07), borderRadius: BorderRadius.circular(10), border: Border.all(color: _T.gold.withOpacity(0.2))), child: Row(children: const [Icon(Icons.star_rounded, color: _T.gold, size: 14), SizedBox(width: 8), Expanded(child: Text('Замена изображений — Premium.', style: TextStyle(color: _T.gold, fontSize: 11, fontWeight: FontWeight.w500)))])), MouseRegion(cursor: isPremium ? SystemMouseCursors.click : SystemMouseCursors.forbidden, child: GestureDetector(onTap: isPremium ? onUpload : null, child: AnimatedContainer(duration: _T.fast, width: double.infinity, height: 90, decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(10), border: Border.all(color: isPremium ? _T.border : _T.border.withOpacity(0.4))), child: Opacity(opacity: isPremium ? 1.0 : 0.4, child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 34, height: 34, decoration: BoxDecoration(color: _T.accentDim, borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.image_rounded, color: _T.accent, size: 17)), const SizedBox(height: 8), const Text('Нажмите для загрузки', style: TextStyle(color: _T.txtSecondary, fontSize: 11)), const Text('PNG, JPG до 10 МБ', style: TextStyle(color: _T.txtMuted, fontSize: 10))]))))), const SizedBox(height: 16), Container(padding: const EdgeInsets.all(12), decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(10), border: Border.all(color: _T.border)), child: const Row(crossAxisAlignment: CrossAxisAlignment.start, children: [Icon(Icons.info_outline_rounded, color: _T.txtMuted, size: 13), SizedBox(width: 8), Expanded(child: Text('Unsplash подбирает изображение.', style: TextStyle(color: _T.txtMuted, fontSize: 11, height: 1.5)))]))]); }

class _AiTab extends StatelessWidget { final bool isImproving; final VoidCallback onImprove; const _AiTab({required this.isImproving, required this.onImprove}); @override Widget build(BuildContext context) => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_SectionLabel('ИИ ПОМОЩНИК'), const SizedBox(height: 12), Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(gradient: LinearGradient(colors: [_T.accent.withOpacity(0.08), _T.accentLight.withOpacity(0.05)], begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: BorderRadius.circular(12), border: Border.all(color: _T.accent.withOpacity(0.2))), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Container(width: 30, height: 30, decoration: BoxDecoration(gradient: const LinearGradient(colors: [_T.accent, _T.accentLight]), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.auto_awesome_rounded, color: Colors.white, size: 15)), const SizedBox(width: 10), const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Улучшить текст', style: TextStyle(color: _T.txtPrimary, fontSize: 13, fontWeight: FontWeight.w700)), Text('Текущий слайд', style: TextStyle(color: _T.txtMuted, fontSize: 10))])]), const SizedBox(height: 10), const Text('ИИ перепишет заголовок и пункты.', style: TextStyle(color: _T.txtSecondary, fontSize: 11, height: 1.5)), const SizedBox(height: 12), SizedBox(width: double.infinity, child: GestureDetector(onTap: isImproving ? null : onImprove, child: AnimatedContainer(duration: _T.fast, padding: const EdgeInsets.symmetric(vertical: 11), decoration: BoxDecoration(gradient: isImproving ? null : const LinearGradient(colors: [Color(0xFF169C46), _T.accent, _T.accentLight], begin: Alignment.centerLeft, end: Alignment.centerRight), color: isImproving ? _T.bgHover : null, borderRadius: BorderRadius.circular(9)), child: Center(child: isImproving ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(color: _T.accent, strokeWidth: 2)) : const Text('Улучшить', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13)))))]))]); }

// ═══════════════════════════════════════════════════════════════════════════════
// EXPORT SHEET (без изменений)
// ═══════════════════════════════════════════════════════════════════════════════
class _ExportSheet extends StatelessWidget { final bool isPremium; final Presentation presentation; const _ExportSheet({required this.isPremium, required this.presentation}); @override Widget build(BuildContext context) => Container(margin: const EdgeInsets.fromLTRB(16, 0, 16, 32), decoration: BoxDecoration(color: _T.bgSurface, borderRadius: _T.r16, border: Border.all(color: _T.border)), child: Column(mainAxisSize: MainAxisSize.min, children: [Padding(padding: const EdgeInsets.only(top: 12, bottom: 16), child: Container(width: 40, height: 4, decoration: BoxDecoration(color: _T.border, borderRadius: BorderRadius.circular(2)))), const Text('Экспорт', style: TextStyle(color: _T.txtPrimary, fontWeight: FontWeight.w700, fontSize: 17)), const SizedBox(height: 4), const Text('Скачайте вашу презентацию', style: TextStyle(color: _T.txtSecondary, fontSize: 12)), const SizedBox(height: 16), Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Column(children: [_ExportOption(icon: Icons.slideshow_rounded, color: _T.accent, title: 'PowerPoint (PPTX)', subtitle: isPremium ? 'Без водяного знака' : 'С водяным знаком', badge: isPremium ? 'PRO' : null, onTap: () { Navigator.pop(context); ExportService.exportToPPTX(context: context, presentation: presentation, isPremium: isPremium); }), const SizedBox(height: 8), _ExportOption(icon: Icons.picture_as_pdf_rounded, color: isPremium ? _T.danger : _T.txtMuted, title: 'PDF', subtitle: isPremium ? 'Высокое качество' : 'Только Premium', locked: !isPremium, onTap: isPremium ? () { Navigator.pop(context); ExportService.exportToPDF(context: context, presentation: presentation, isPremium: true); } : null)])), const SizedBox(height: 20)])); }
class _ExportOption extends StatelessWidget { final IconData icon; final Color color; final String title, subtitle; final String? badge; final bool locked; final VoidCallback? onTap; const _ExportOption({required this.icon, required this.color, required this.title, required this.subtitle, this.badge, this.locked = false, this.onTap}); @override Widget build(BuildContext context) => GestureDetector(onTap: onTap, child: Opacity(opacity: locked ? 0.5 : 1.0, child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12), decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(12), border: Border.all(color: _T.border)), child: Row(children: [Container(width: 40, height: 40, decoration: BoxDecoration(color: color.withOpacity(0.12), borderRadius: BorderRadius.circular(10)), child: Icon(icon, color: color, size: 20)), const SizedBox(width: 12), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(color: _T.txtPrimary, fontWeight: FontWeight.w600, fontSize: 14)), const SizedBox(height: 2), Text(subtitle, style: const TextStyle(color: _T.txtSecondary, fontSize: 11))])), if (badge != null) Container(padding: const EdgeInsets.symmetric(horizontal: 7, vertical: 3), decoration: BoxDecoration(gradient: const LinearGradient(colors: [_T.accent, _T.accentLight]), borderRadius: BorderRadius.circular(5)), child: Text(badge!, style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w800))), if (locked) const Icon(Icons.lock_rounded, color: _T.txtMuted, size: 16), if (!locked && badge == null) const Icon(Icons.arrow_forward_ios_rounded, color: _T.txtMuted, size: 12)])))); }

// ═══════════════════════════════════════════════════════════════════════════════
// CONTROL BAR (без изменений)
// ═══════════════════════════════════════════════════════════════════════════════
class _ControlBar extends StatelessWidget { final int activeSlide, totalSlides; final bool propsPanelOpen; final VoidCallback onToggleProps, onPrev, onNext, onAdd, onDelete, onDuplicate; const _ControlBar({required this.activeSlide, required this.totalSlides, required this.propsPanelOpen, required this.onToggleProps, required this.onPrev, required this.onNext, required this.onAdd, required this.onDelete, required this.onDuplicate}); @override Widget build(BuildContext context) => Container(height: 44, color: _T.bgSurface, padding: const EdgeInsets.symmetric(horizontal: 16), child: Row(children: [_IconBtn(Icons.undo_rounded, () {}, size: 16, tooltip: 'Отменить'), _IconBtn(Icons.redo_rounded, () {}, size: 16, tooltip: 'Повторить'), const SizedBox(width: 8), Container(width: 1, height: 20, color: _T.border), const SizedBox(width: 8), _IconBtn(Icons.copy_rounded, onDuplicate, size: 15, tooltip: 'Дублировать'), _IconBtn(Icons.delete_outline_rounded, onDelete, size: 15, tooltip: 'Удалить', danger: true), const Spacer(), _IconBtn(Icons.arrow_back_rounded, onPrev, size: 16, disabled: activeSlide == 0), Padding(padding: const EdgeInsets.symmetric(horizontal: 10), child: Text('${activeSlide + 1} / $totalSlides', style: const TextStyle(color: _T.txtSecondary, fontSize: 12, fontWeight: FontWeight.w600))), _IconBtn(Icons.arrow_forward_rounded, onNext, size: 16, disabled: activeSlide == totalSlides - 1), const Spacer(), _IconBtn(propsPanelOpen ? Icons.view_sidebar : Icons.view_sidebar_outlined, onToggleProps, size: 16, tooltip: propsPanelOpen ? 'Скрыть панель' : 'Показать панель', active: propsPanelOpen)])); }

// ═══════════════════════════════════════════════════════════════════════════════
// SHARED WIDGETS (без изменений)
// ═══════════════════════════════════════════════════════════════════════════════
class _SectionLabel extends StatelessWidget { final String text; const _SectionLabel(this.text); @override Widget build(BuildContext context) => Text(text, style: const TextStyle(color: _T.txtMuted, fontSize: 10, fontWeight: FontWeight.w700, letterSpacing: 0.8)); }

class _IconBtn extends StatefulWidget { final IconData icon; final VoidCallback? onTap; final double size; final String? tooltip; final bool danger, active, disabled; const _IconBtn(this.icon, this.onTap, {this.size = 18, this.tooltip, this.danger = false, this.active = false, this.disabled = false}); @override State<_IconBtn> createState() => _IconBtnState(); }
class _IconBtnState extends State<_IconBtn> { bool _hovered = false; @override Widget build(BuildContext context) { final color = widget.disabled ? _T.txtMuted : widget.danger ? _T.danger : widget.active ? _T.accent : _hovered ? _T.txtPrimary : _T.txtSecondary; return MouseRegion(cursor: widget.disabled ? SystemMouseCursors.forbidden : SystemMouseCursors.click, onEnter: (_) => setState(() => _hovered = true), onExit: (_) => setState(() => _hovered = false), child: Tooltip(message: widget.tooltip ?? '', child: GestureDetector(onTap: widget.disabled ? null : widget.onTap, child: AnimatedContainer(duration: _T.fast, padding: const EdgeInsets.all(7), decoration: BoxDecoration(color: _hovered && !widget.disabled ? _T.bgHover : Colors.transparent, borderRadius: BorderRadius.circular(7)), child: Icon(widget.icon, size: widget.size, color: color))))); } }

extension _IterableMapIndexed<T> on Iterable<T> { Iterable<R> mapIndexed<R>(R Function(int index, T element) f) { var i = 0; return map((e) => f(i++, e)); } }