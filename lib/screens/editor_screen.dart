...
class _SlideNavigator extends StatelessWidget {
  ...
  const _SlideNavigator({
    required this.slides, required this.titleControllers, required this.activeIndex, required this.collapsed,
    required this.customBgs, required this.backgrounds, required this.selectedBgIndex, required this.onSelect,
    required this.onAdd, required this.onDelete, required this.onDuplicate, required this.onMoveUp,
    required this.onMoveDown, required this.onToggleCollapse,
  });
  Color _getColor(int i) {
    if (customBgs[i] != null) return Colors.grey.shade800;
    var bg = backgrounds[selectedBgIndex];
...
class _Canvas extends StatelessWidget {
  ...
  const _Canvas({
    super.key, required this.index, required this.titleCtrl, required this.contentCtrl, required this.decoration,
    required this.font, required this.fontSize, required this.fontColor, required this.slideCount,
    required this.textStyle, required this.textAlign, required this.columnsCount, this.image, this.chartType,
    required this.chartData, required this.shapes, required this.imageWidth, required this.imageHeight,
    required this.imagePosition, required this.imageTextWrap, required this.onAddItem, required this.onRemoveItem,
    required this.onRemoveImage, required this.hasCustomImage,
  });
            ...
            return Container(width: width, height: height, decoration: decoration, clipBehavior: Clip.antiAlias,
              child: Stack(children: [
                ...
                Positioned(top: 12, left: 14, child: Container(
                  ...
                  child: Text('${index + 1}', style: const TextStyle(color: Colors.white60, fontSize: 9, fontWeight: FontWeight.w700)),
                )),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        TextField(
          ...
          style: GoogleFonts.inter(color: _T.txtPrimary, fontSize: 15, fontWeight: FontWeight.w700),
          maxLines: null,
          decoration: InputDecoration(
            hintText: 'Заголовок...', hintStyle: GoogleFonts.inter(color: _T.txtMuted),
            filled: true, fillColor: _T.bgCard, contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
        ...
        ...contentCtrl.asMap().entries.map((e) => Padding(padding: const EdgeInsets.only(bottom: 6), child: Row(children: [
          ...
          Expanded(child: TextField(
            controller: e.value,
            style: GoogleFonts.inter(color: _T.txtPrimary, fontSize: 13),
            maxLines: null,
            decoration: InputDecoration(
        ...
        GestureDetector(onTap: onAddItem, child: const Row(mainAxisSize: MainAxisSize.min, children: [
          Icon(Icons.add_rounded, color: _T.accent, size: 14), SizedBox(width: 4),
          Text('Добавить пункт', style: TextStyle(color: _T.accent, fontSize: 12)),
        ])),
      ]),
    );
  }
...
class _StarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = color..style = PaintingStyle.fill;
    final path = Path();
    double cx = size.width / 2, cy = size.height / 2, or = size.width / 2, ir = or * 0.4;
    for (int i = 0; i < 10; i++) {
...
class _PropertiesPanel extends StatelessWidget {
  ...
  const _PropertiesPanel({
    ...
    required this.selectedBgIndex, required this.freeBgs, required this.premiumBgs, required this.customBg,
    required this.fontSize, required this.fontColor, required this.transition, required this.allTransitions,
    required this.isImproving, required this.currentTextStyle, required this.currentTextAlign,
    required this.columnsCount, required this.textStyles, this.chartType, required this.chartData,
    required this.shapes, this.imageWidth, this.imageHeight, this.imagePosition, this.imageTextWrap,
    required this.hasImage, required this.onTabChange, required this.onBgSelect, required this.onBgUpload,
    required this.onImageUpload, required this.onFontChange, required this.onFontSizeChange,
    required this.onFontColorChange, required this.onTransitionChange, required this.onTextStyleChange,
    required this.onTextAlignChange, required this.onColumnsChange, required this.onChartTypeChange,
    required this.onChartDataChange, required this.onAddShape, required this.onRemoveShape,
    required this.onImageWidthChange, required this.onImageHeightChange, required this.onImagePositionChange,
    required this.onImageTextWrapChange, required this.uploadsUsed, required this.onImprove,
  });

  @override
  Widget build(BuildContext context) => Container(color: _T.bgSurface, child: Column(children: [
    Container(height: 40, padding: const EdgeInsets.symmetric(horizontal: 6), child: Row(children: [
      _buildTab('design', 'Дизайн', Icons.palette_rounded),
      _buildTab('image', 'Медиа', Icons.image_rounded),
      _buildTab('shapes', 'Фигуры', Icons.shape_line_rounded),
      _buildTab('charts', 'Графики', Icons.show_chart_rounded),
      _buildTab('ai', 'ИИ', Icons.auto_awesome_rounded),
    ])),
    const Divider(color: _T.border, height: 1),
    Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(14), child: _buildContent())),
  ]));

  Widget _buildTab(String id, String label, IconData icon) => Expanded(child: GestureDetector(
    onTap: () => onTabChange(id),
    child: AnimatedContainer(
      duration: _T.fast, margin: const EdgeInsets.symmetric(horizontal: 2, vertical: 6), padding: const EdgeInsets.symmetric(horizontal: 4),
      decoration: BoxDecoration(color: id == activeTab ? _T.accentDim : Colors.transparent, borderRadius: BorderRadius.circular(6), border: Border.all(color: id == activeTab ? _T.accent.withOpacity(0.3) : Colors.transparent)),
      child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(icon, size: 12, color: id == activeTab ? _T.accentLight : _T.txtMuted),
        const SizedBox(width: 4),
        Text(label, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: id == activeTab ? _T.accentLight : _T.txtMuted)),
      ]),
    ),
  ));
  ...
  Widget _buildDesignTab() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Wrap(spacing: 7, children: [Colors.white, Colors.black, _T.accent, Colors.blue, Colors.red, _T.gold].map((c) => GestureDetector(
      onTap: () => onFontColorChange(c),
      child: Container(width: 28, height: 28, decoration: BoxDecoration(color: c, shape: BoxShape.circle, border: Border.all(color: fontColor == c ? _T.accent : Colors.transparent, width: 2))),
    )).toList()),
    const SizedBox(height: 16),
    const Text('ФОН', style: TextStyle(color: _T.txtMuted, fontSize: 10, fontWeight: FontWeight.w700)),
    Wrap(spacing: 8, children: freeBgs.asMap().entries.map((e) => GestureDetector(
      onTap: () => onBgSelect(e.key),
      child: Container(width: 44, height: 44, decoration: BoxDecoration(
        gradient: e.value['type'] == 'gradient' ? LinearGradient(colors: e.value['colors'] as List<Color>) : null,
        color: e.value['type'] == 'solid' ? e.value['color'] as Color : null,
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: selectedBgIndex == e.key && customBg == null ? _T.accent : Colors.transparent, width: 2),
      )),
    )).toList()),
    const SizedBox(height: 8),
  ...
  Widget _buildShapesTab() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Wrap(spacing: 8, children: [
      ...
      child: Container(width: 50, height: 50, decoration: BoxDecoration(color: _T.bgCard, borderRadius: BorderRadius.circular(8)), child: Icon(pair.$2, color: _T.accent, size: 28)),
    )).toList()),
    if (shapes.isNotEmpty) ...[
      ...
      ...shapes.map((s) => Container(
        ...
        child: Row(children: [
          Icon(_getShapeIcon(s.type), color: s.color),
          const SizedBox(width: 12),
          Expanded(child: Text(s.type, style: const TextStyle(color: _T.txtPrimary))),
          GestureDetector(onTap: () => onRemoveShape(s.id), child: const Icon(Icons.close, color: _T.danger)),
        ]),
      )),
    ],
  ]);

  IconData _getShapeIcon(String t) {
    switch (t) {
  ...
  Widget _buildChartsTab() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
      _chartButton('bar', Icons.bar_chart_rounded, 'Столбчатый'),
      _chartButton('pie', Icons.pie_chart_rounded, 'Круговой'),
      _chartButton('line', Icons.show_chart_rounded, 'Линейный'),
      _chartButton(null, Icons.clear_rounded, 'Удалить'),
    ]),
    if (chartType != null) ...[
      const SizedBox(height: 16),
      const Text('ДАННЫЕ', style: TextStyle(color: _T.txtMuted, fontSize: 10, fontWeight: FontWeight.w700)),
      ...chartData.asMap().entries.map((e) => Row(children: [
        Expanded(child: TextField(
          controller: TextEditingController(text: e.value['label']),
          style: const TextStyle(color: Colors.white),
          onChanged: (v) { var d = List<Map<String, dynamic>>.from(chartData); d[e.key]['label'] = v; onChartDataChange(d); },
        )),
        const SizedBox(width: 8),
        Expanded(child: TextField(
          ...
          style: const TextStyle(color: Colors.white),
          onChanged: (v) { var d = List<Map<String, dynamic>>.from(chartData); d[e.key]['value'] = double.tryParse(v) ?? 0; onChartDataChange(d); },
        )),
      ])),
      GestureDetector(
        onTap: () { var d = List<Map<String, dynamic>>.from(chartData); d.add({'label': 'Новый', 'value': 100.0}); onChartDataChange(d); },
        child: Container(padding: const EdgeInsets.symmetric(vertical: 8), decoration: BoxDecoration(color: _T.accentDim, borderRadius: BorderRadius.circular(8)), child: const Center(child: Text('+ Добавить', style: TextStyle(color: _T.accent)))),
      ),
    ],
  ]);
  ...
  Widget _chartButton(String? t, IconData icon, String label) => GestureDetector(
    ...
    child: Column(children: [
      Container(width: 50, height: 50, decoration: BoxDecoration(color: chartType == t ? _T.accentDim : _T.bgCard, borderRadius: BorderRadius.circular(8), border: Border.all(color: chartType == t ? _T.accent : _T.border)), child: Icon(icon, color: chartType == t ? _T.accent : _T.txtSecondary)),
      const SizedBox(height: 4),
      Text(label, style: TextStyle(fontSize: 10, color: chartType == t ? _T.accent : _T.txtSecondary)),
    ]),
  );
  ...
  Widget _buildAiTab() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    ...
    Container(
      ...
      decoration: BoxDecoration(gradient: LinearGradient(colors: [_T.accent.withOpacity(0.08), _T.accentLight.withOpacity(0.05)]), borderRadius: BorderRadius.circular(12)),
      child: Column(children: [
        Row(children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(gradient: const LinearGradient(colors: [_T.accent, _T.accentLight]), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.auto_awesome, color: Colors.white, size: 18)),
          const SizedBox(width: 12),
          const Text('Улучшить текст', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700)),
        ]),
        const SizedBox(height: 12),
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(gradient: isImproving ? null : const LinearGradient(colors: [_T.accent, _T.accentLight]), borderRadius: BorderRadius.circular(8)),
            child: Center(child: isImproving ? const SizedBox(width: 20, height: 20, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('Улучшить', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w700))),
          ),
        ),
      ]),
    ),
  ...
  Widget build(BuildContext context) => Container(
    ...
    child: Column(mainAxisSize: MainAxisSize.min, children: [
      ...
      ListTile(
        ...
        trailing: !isPremium ? const Icon(Icons.lock) : null,
      ),
      const SizedBox(height: 16),
    ]),
  );
...
class _ControlBar extends StatelessWidget {
  ...
  const _ControlBar({
    required this.activeSlide, required this.totalSlides, required this.propsPanelOpen,
    required this.onToggleProps, required this.onPrev, required this.onNext, required this.onAdd,
    required this.onDelete, required this.onDuplicate, required this.onTemplate,
  });
  @override
  Widget build(BuildContext context) => Container(
    height: 56,
    ...
    child: Row(children: [
      ...
      _IconBtn(propsPanelOpen ? Icons.view_sidebar : Icons.view_sidebar_outlined, onToggleProps),
    ]),
  );
}
    ...
    return MouseRegion(
      ...
      child: Tooltip(
        ...
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            duration: _T.fast, padding: const EdgeInsets.all(8),
            decoration: BoxDecoration(color: hover && !widget.disabled ? _T.bgHover : Colors.transparent, borderRadius: BorderRadius.circular(8)),
