...
class _EditorScreenState extends State<EditorScreen>
  ...
  String? get _logoUrl => Provider.of<LogoProvider>(context, listen: true).logoUrl;
  ...
  void _moveSlide(int from, int to) {
    ...
    setState(() {
      void swap<T>(List<T> l) { final t = l[from]; l[from] = l[to]; l[to] = t; }
      swap(_presentation.slides); swap(_titleCtrl); swap(_contentCtrl);
      swap(_customImages); swap(_customBgs); swap(_fontSizes);
      swap(_fonts); swap(_slideFontColors); swap(_transitions);
      _activeSlide = to;
    });
  }
  ...
  void _removeContentItem(int slide, int item) {
    ...
    setState(() { _contentCtrl[slide][item].dispose(); _contentCtrl[slide].removeAt(item); });
  }
  ...
  Future<void> _uploadImage(int index) async {
    ...
    if (!p) { _toast('Замена картинок — Premium', warning: true); return; }
    final input = html.FileUploadInputElement()..accept = 'image/*';
    input.click();
    input.onChange.listen((e) {
      final f = input.files?.first; if (f == null) return;
      final r = html.FileReader(); r.readAsDataUrl(f);
      r.onLoad.listen((_) => setState(() => _customImages[index] = r.result as String));
    });
  }
  ...
  Future<void> _uploadBg(int index) async {
    ...
    if (!p && used >= 10 && _customBgs[index] == null) { _toast('10 фонов бесплатно', warning: true); return; }
    final input = html.FileUploadInputElement()..accept = 'image/*';
    input.click();
    input.onChange.listen((e) {
      final f = input.files?.first; if (f == null) return;
      final r = html.FileReader(); r.readAsDataUrl(f);
      r.onLoad.listen((_) => setState(() => _customBgs[index] = r.result as String));
    });
  }

  Decoration _slideDeco(int index) {
    if (_customBgs[index] != null) return BoxDecoration(image: DecorationImage(image: NetworkImage(_customBgs[index]!), fit: BoxFit.cover), borderRadius: _T.r12);
    final bg = _freeBgs[_selectedBgIndex.clamp(0, _freeBgs.length - 1)];
    if (bg['type'] == 'gradient') return BoxDecoration(gradient: LinearGradient(colors: bg['colors'] as List<Color>, begin: Alignment.topLeft, end: Alignment.bottomRight), borderRadius: _T.r12);
  ...
  void _export() { _saveAll(); final p = Provider.of<UserProvider>(context, listen: false).isPremium; _showSheet(_ExportSheet(isPremium: p, presentation: _presentation)); }
  ...
  Widget build(BuildContext context) {
    ...
    if (_slideFontColors[_activeSlide] == null) _globalFontColor = dark ? Colors.white : Colors.black;
    final logo = _logoUrl;
    ...
    return Scaffold(
      ...
      body: Column(children: [
        ...
        Expanded(child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          AnimatedContainer(duration: _T.normal, width: _navCollapsed ? 48 : 200, child: _SlideNavigator(slides: _presentation.slides, titleControllers: _titleCtrl, activeIndex: _activeSlide, collapsed: _navCollapsed, customBgs: _customBgs, backgrounds: _freeBgs, selectedBgIndex: _selectedBgIndex, onSelect: (i) => setState(() => _activeSlide = i), onAdd: _addSlide, onDelete: _deleteSlide, onDuplicate: _duplicateSlide, onMoveUp: (i) => _moveSlide(i, i - 1), onMoveDown: (i) => _moveSlide(i, i + 1), onToggleCollapse: () => setState(() => _navCollapsed = !_navCollapsed))),
          const VerticalDivider(color: _T.border, width: 1),
          Expanded(child: _Canvas(key: _canvasKey, index: _activeSlide, titleCtrl: _titleCtrl[_activeSlide], contentCtrl: _contentCtrl[_activeSlide], decoration: _slideDeco(_activeSlide), isDark: _isDark(_activeSlide), image: _customImages[_activeSlide] ?? _autoImages[_activeSlide], font: _fonts[_activeSlide] != 'Inter' ? _fonts[_activeSlide] : _globalFont, fontSize: _fontSizes[_activeSlide], fontColor: _slideFontColors[_activeSlide] ?? _globalFontColor, slideCount: _presentation.slides.length, logoUrl: logo, onAddItem: () => _addContentItem(_activeSlide), onRemoveItem: (i) => _removeContentItem(_activeSlide, i), onRemoveImage: () => setState(() { _customImages[_activeSlide] = null; _countUploads(); }), hasCustomImage: _customImages[_activeSlide] != null)),
          const VerticalDivider(color: _T.border, width: 1),
          AnimatedContainer(duration: _T.normal, width: _propsPanelOpen ? 260 : 0, child: _propsPanelOpen ? _PropertiesPanel(index: _activeSlide, isPremium: Provider.of<UserProvider>(context, listen: false).isPremium, activeTab: _activePropTab, globalFont: _globalFont, selectedBgIndex: _selectedBgIndex, freeBgs: _freeBgs, premiumBgs: _premiumBgs, customBg: _customBgs[_activeSlide], fontSize: _fontSizes[_activeSlide], fontColor: _slideFontColors[_activeSlide] ?? _globalFontColor, transition: _transitions[_activeSlide], allTransitions: _allTransitions, isImproving: _isImproving, onTabChange: (t) => setState(() => _activePropTab = t), onBgSelect: (i) => setState(() { _selectedBgIndex = i; _customBgs = List.filled(_presentation.slides.length, null); }), onBgUpload: () => _uploadBg(_activeSlide), onImageUpload: () => _uploadImage(_activeSlide), onFontChange: (f) => setState(() { _globalFont = f; for (int i = 0; i < _fonts.length; i++) _fonts[i] = f; }), onFontSizeChange: (v) => setState(() => _fontSizes[_activeSlide] = v), onFontColorChange: (c) => setState(() { _slideFontColors[_activeSlide] = c; _globalFontColor = c; }), onTransitionChange: (t) => setState(() => _transitions[_activeSlide] = t), uploadsUsed: _imageUploadsUsed) : const SizedBox.shrink()),
        ])),
      ]),
    );
  }
...
class _TopBar extends StatelessWidget {
  final String title; final int slideCount; final int uploadsUsed; final VoidCallback onBack; final VoidCallback onExport;
  ...
  @override
  Widget build(BuildContext context) {
    return Container(height: 52, color: _T.bgSurface, padding: const EdgeInsets.symmetric(horizontal: 12), child: Row(children: [
...
class _SlideNavigator extends StatelessWidget {
  ...
  final ValueChanged<int> onSelect; final VoidCallback onAdd; final ValueChanged<int> onDelete; final ValueChanged<int> onDuplicate;
  final ValueChanged<int> onMoveUp; final ValueChanged<int> onMoveDown; final VoidCallback onToggleCollapse;
  ...
  @override
  Widget build(BuildContext context) => Container(color: _T.bgSurface, child: Column(children: [
...
class _SlideThumbnail extends StatefulWidget {
  final int index; final String title; final bool isActive; final bool collapsed; final Color bgColor;
  final VoidCallback onTap; final VoidCallback? onDelete; final VoidCallback onDuplicate; final VoidCallback? onMoveUp; final VoidCallback? onMoveDown;
...
class _Canvas extends StatelessWidget {
  final int index; final TextEditingController titleCtrl; final List<TextEditingController> contentCtrl;
  final Decoration decoration; final bool isDark; final String? image; final String font;
  final double fontSize; final Color fontColor; final int slideCount; final String? logoUrl;
  final VoidCallback onAddItem; final ValueChanged<int> onRemoveItem; final VoidCallback onRemoveImage; final bool hasCustomImage;
  ...
  @override
  Widget build(BuildContext context) => Container(color: _T.bgBase, child: Center(child: SingleChildScrollView(padding: const EdgeInsets.symmetric(vertical: 32, horizontal: 24), child: Column(children: [
    ...
    LayoutBuilder(builder: (ctx, constraints) {
      ...
      return Container(width: width, height: height, decoration: decoration, clipBehavior: Clip.antiAlias, child: Stack(children: [
        ...
        if (logoUrl != null)
          Positioned(bottom: 10, right: 14, child: Opacity(opacity: 0.7, child: ClipRRect(borderRadius: BorderRadius.circular(4), child: Image.network(logoUrl!, width: 50, height: 20, fit: BoxFit.contain, errorBuilder: (_, __, ___) => const SizedBox())))),
      ]));
    }),
...
class _EditorField extends StatelessWidget {
  final TextEditingController controller; final String hint; final bool isTitle;
...
class _PropertiesPanel extends StatelessWidget {
  final int index; final bool isPremium; final String activeTab; final String globalFont; final int selectedBgIndex;
  final List<Map<String, dynamic>> freeBgs; final List<Map<String, dynamic>> premiumBgs; final String? customBg;
  final double fontSize; final Color fontColor; final String transition; final List<Map<String, dynamic>> allTransitions;
  final bool isImproving; final ValueChanged<String> onTabChange; final ValueChanged<int> onBgSelect; final VoidCallback onBgUpload;
  final VoidCallback onImageUpload; final ValueChanged<String> onFontChange; final ValueChanged<double> onFontSizeChange;
  final ValueChanged<Color> onFontColorChange; final ValueChanged<String> onTransitionChange; final int uploadsUsed;
  ...
  Widget build(BuildContext context) => Container(color: _T.bgSurface, child: Column(children: [
    ...
    Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(14), child: switch (activeTab) {
      ...
      _       => _DesignTab(globalFont: globalFont, selectedBgIndex: selectedBgIndex, freeBgs: freeBgs, premiumBgs: premiumBgs, customBg: customBg, fontSize: fontSize, fontColor: fontColor, transition: transition, allTransitions: allTransitions, isPremium: isPremium, onBgSelect: onBgSelect, onBgUpload: onBgUpload, onFontChange: onFontChange, onFontSizeChange: onFontSizeChange, onFontColorChange: onFontColorChange, onTransitionChange: onTransitionChange),
    })),
  ]));
}

class _Tab extends StatelessWidget {
  final String id, label; final IconData icon; final String active; final ValueChanged<String> onTap;
...
class _DesignTab extends StatelessWidget {
  final String globalFont; final int selectedBgIndex; final List<Map<String, dynamic>> freeBgs; final List<Map<String, dynamic>> premiumBgs;
  final String? customBg; final double fontSize; final Color fontColor; final String transition; final List<Map<String, dynamic>> allTransitions;
  final bool isPremium; final ValueChanged<int> onBgSelect; final VoidCallback onBgUpload; final ValueChanged<String> onFontChange;
  final ValueChanged<double> onFontSizeChange; final ValueChanged<Color> onFontColorChange; final ValueChanged<String> onTransitionChange;
  ...
  static const List<Color> _fontColors = [Colors.white, Color(0xFFF2F2F2), Color(0xFF1A1A2E), Colors.black, Color(0xFF1DB954), Color(0xFF4facfe), Color(0xFFf5576c), Color(0xFFFFD700), Color(0xFFf093fb), Color(0xFFFF6B35)];
...
class _ImageTab extends StatelessWidget {
  final VoidCallback onUpload; final bool isPremium;
...
class _ExportSheet extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Container(margin: const EdgeInsets.fromLTRB(16, 0, 16, 32), decoration: BoxDecoration(color: _T.bgSurface, borderRadius: _T.r16, border: Border.all(color: _T.border)), child: Column(mainAxisSize: MainAxisSize.min, children: [
    ...
    const Text('Экспорт', style: TextStyle(color: _T.txtPrimary, fontWeight: FontWeight.w700, fontSize: 17)), const SizedBox(height: 4),
    const Text('Скачайте вашу презентацию', style: TextStyle(color: _T.txtSecondary, fontSize: 12)), const SizedBox(height: 16),
    Padding(padding: const EdgeInsets.symmetric(horizontal: 16), child: Column(children: [
      _ExportOption(icon: Icons.slideshow_rounded, color: _T.accent, title: 'PPTX', subtitle: isPremium ? 'Без знака' : 'С водяным знаком', badge: isPremium ? 'PRO' : null, onTap: () { Navigator.pop(context); ExportService.exportToPPTX(context: context, presentation: presentation, isPremium: isPremium); }),
      const SizedBox(height: 8),
...
class _ExportOption extends StatelessWidget {
  final IconData icon; final Color color; final String title; final String subtitle; final String? badge; final bool locked; final VoidCallback? onTap;
...
class _IconBtn extends StatefulWidget {
  final IconData icon; final VoidCallback? onTap; final double size; final String? tooltip; final bool danger; final bool active; final bool disabled;
...
extension _IterableMapIndexed<T> on Iterable<T> {
  Iterable<R> mapIndexed<R>(R Function(int index, T element) f) { var i = 0; return map((e) => f(i++, e)); }
}